...
//                mainBinding.taskRV.smoothScrollToPosition(positionStart)
    ...
    private fun callSortByDialog() {
        ...
        val items = arrayOf("Titre Croissant", "Titre Décroissant","Date Croissant","Date Décroissant")

        mainBinding.sortImg.setOnClickListener {
            MaterialAlertDialogBuilder(this)
                .setTitle("Trier Par")
                .setPositiveButton("Trier") { _, _ ->
                    when (checkedItem) {
                        0 -> {
                ...
                when (it.status) {
                    ...
                    Status.SUCCESS -> {
                        ...
                        when (it.data as StatusResult) {
                            Added -> {
                                Log.d("StatusResult", "Ajoutée")
                            }

                            Deleted -> {
                                Log.d("StatusResult", "Supprimée")

                            }

                            Updated -> {
                                Log.d("StatusResult", "Modifiée")
